  ...
  stages {
    ...
    stage('Deploy') {
      ...
      steps {
        ...
        sh 'ssh -o StrictHostKeyChecking=no ubuntu@${SERVER_IP} \"rm -rf ${SERVER_DEPLOY_DIR}/build; rm -rf ${SERVER_DEPLOY_DIR}/build_$((BUILD_NUMBER - 3)).tar; rm -rf; tar -xvf ${SERVER_DEPLOY_DIR}/build_${BUILD_NUMBER}.tar -C ${SERVER_DEPLOY_DIR}\"'
        echo '--- Deploy end ---'
      }
    }
  }
